...
actor.rules = {
  ...
  jmpatk = { ActorScript.isFrame(3) / ActorScript.hitAll{dmg = 8, force = {x = 120}},
            jmp = ActorScript.isEnded,
            std = SceneScript.isFloor },
  runend = { std = ActorScript.isEnded },
  ...
  hitflr = { std = ActorScript.isEnded,
             die = ActorScript.isDied },
  hitalt = { hitair = -SceneScript.isFloor,
             std = ActorScript.isEnded },
  hithvy = { hitair = ActorScript.isFall },
}

actor.keybrules = {
  std = { blk = ActorScript.isKey{"ad>"},
  ...
  wlk = { std = -ActorScript.isKey{"[rlud]*b>", "r>r>", "l>l>", "[rlud]+>", "au>u>d>"},
  ...
  jmp = { jmpatk = ActorScript.isKey{"a[rlud]*>"} },
  
  run = { run = ActorScript.isKey{"r>r>", "l>l>"},
          ...
          runend = -ActorScript.isKey{"r>r>", "l>l>", "[rlud]*b>", "au>u>d>", "ar>r>r>", "al>l>l>"} },
        
  blk = { blk =  ActorScript.isKey{"ad>"},
          std = -ActorScript.isKey{"ad>"} },
        
  atk = { atksq1 = ActorScript.isKey{"a>a>"} ^ ActorScript.isFrame(7) },
  atksq1 = { atksq2 = ActorScript.isKey{"a>a>a>"} ^ ActorScript.isFrame(8) },
  atksq2 = { atksq3 = ActorScript.isKey{"a>a>a>a>"} ^ ActorScript.isFrame(5) },
}
